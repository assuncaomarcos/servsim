...
/**
 * Class responsible for managing a simulation. A simulation has
 * basically two event queues, one for future events and another
 * for deferred events. The entities create events and add them
 * to the future queue. The simulation constantly checks the
 * future event queue, removes events that can be handled at a
 * given clock tick and adds them to the deferred event queue.
 * At each clock tick, after events are added to the deferred queue,
 * the entities responsible for handling them are invoked.
 * This is performed by calling {@link SimEntity#process(SimEvent)}
 * of each entity object that must handle an event in the deferred queue.
 * <p>
...
public abstract class Simulation implements Runnable {
    ...
    private static final Logger LOGGER =
            LogManager.getLogger(Simulation.class.getName());

    /**
     * The future event queue.
     */
    private TreeSet<SimEvent> future = new TreeSet<>();

    /**
     * List of events being handled at a clock tick.
     */
    private LinkedList<SimEvent> deferred = new LinkedList<>();
    ...
    /**
     * Comparator used to sort events by priority.
     */
    ...
    /**
     * Defines whether the simulation can be abruptly stopped.
     */
    private boolean abruptInterrupt = false;

    /**
     * Time the warm-up period ends.
     */
     ...
     */
    public void setTimeSpan(final long span,
                            final boolean abruptInterrupt)
            throws IllegalArgumentException,
            IllegalStateException {

        checkArgument(span > 0,
                "Span needs to be greater than 0");
        checkArgument(timeSpan == Long.MAX_VALUE,
                "Span already set: %s", timeSpan);
     ...
     * @return <code>true</code> if the simulation
     * is running; <code>false</code> otherwise.
     */
     ...
     */
    public boolean registerEntity(final SimEntity entity) {
        checkState(entity.getSimulation() == null,
                "Entity already registered with a simulation");
    ...
    /**
     * Sets the clock unit to be used by the simulator.
     ...
     */
    public void setClockUnit(final TimeUnit unit) {
        clock.setUnit(unit);
    }
    ...
    public void setStartDate(final Date date) {
        checkState(!isRunning(),
                "Error setting start date, simulation is running");
        clock.setStartDate(checkNotNull(date));
    }

    /**
     * Sets a comparator to sort events handled at each
     * simulation clock tick.
     * @param comp the event comparator
     */
    public void setClockTickEventComparator(
            final Comparator<SimEvent> comp) {
     ...
     */
    public SimEntity getEntity(final int id) {
     ...
     * @return the entity or <code>null</code> if not found.
     */
    public SimEntity getEntity(final String name) {
     ...
     */
    public List<SimEntity> getEntities(final Predicate<SimEntity> filter) {
     ...
     */
    public int countFutureEvents(final Predicate<SimEvent> filter) {
        int count = 0;
     ...
     */
    public boolean hasFutureEvent(final Predicate<SimEvent> filter) {
     ...
     * @param filter the filter to be used
     */
    public void cancelFutureEvents(final Predicate<SimEvent> filter) {
     ...
     */
    public void cancelNextFutureEvent(final Predicate<SimEvent> filter) {
        Iterator<SimEvent> it = future.iterator();
        while (it.hasNext()) {
            SimEvent ev = it.next();
            if (filter.test(ev)) {
    ...
    private boolean processEvents() {
        while (!isPaused() && (clock.getTime() < this.timeSpan
                || !this.abruptInterrupt)) {
    ...
    /**
     * Used by an entity to send an event to another entity.
     * @param src the source entity's id
     * @param dest the destination entity's id
     * @param delay the simulation delay
     * @param type the type of event
     * @param content the content of the event
     */
    @SuppressWarnings("rawtypes")
    void send(final int src, final int dest,
              final long delay, final Enum type,
              final Object content) {
        checkArgument(delay >= 0L,
                "Send delay must be >= 0.");
        future.add(new SimEvent(type,
                content, clock.getTime() + delay, src, dest));
    }
     ...
     */
    private void processEvent(final SimEvent ev) {
        int dest;
        // Update the system's clock
        if (ev.time() < clock.getTime()) {
            throw new IllegalArgumentException(
                "The event was scheduled for the past.");
        }
    ...
    /**
     * Class to which entity management is delegated. This is to
     * avoid that users mess up with the number of entities in the
     * system and how they get created or deleted. So the whole
     * thing must be managed by Simulation class.
     */
    private static class EntityList extends ArrayList<SimEntity> {
        @Serial
        private static final long serialVersionUID =
                6922342328272754582L;

        /**
         * Adds an entity to the simulation.
         * @param entity the entity to add
         * @return <code>true</code> if the entity was added.
         */
        boolean addEntity(final SimEntity entity) {
            return super.add(entity);
        }

        /**
         * Returns an array of entities that match the filtering
         * criteria.
         * @param filter a predicate used for filtering out entities
         * @return the list of entities
         */
        List<SimEntity> getEntities(final Predicate<SimEntity> filter) {
         ...
         * @return the entity or <code>null</code> if not found.
         */
        SimEntity getEntity(final String name) {
        ...
        SimEntity getEntity(final int id) {
            for (SimEntity entity : this) {
                if (entity.getId() == id) {
                    return entity;
                }
        ...
        private List<SimEntity> filter(
                final List<SimEntity> thelist,
                final Predicate<SimEntity> p) {
