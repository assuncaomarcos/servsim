...
public class PartProfile extends Profile<PartProfileEntry>
        implements Iterable<PartProfileEntry> {

    /** The resource partitions. */
     ...
     */
    public PartProfile(final ResourcePartition[] parts) {
        this(parts.length);
     ...
     * @see PartProfileEntry
     */
    private PartProfile(final LinkedTreeMap<Long, PartProfileEntry> avail,
                        final ResourcePartition[] parts) {
     ...
     */
    private PartProfile(final int numParts) {
     ...
     * can handle the job.
     */
    public int matchPartition(final WorkUnit item) {
     ...
     */
    public ProfileEntry checkPartAvailability(final int partId,
                                              final long time) {
        Map.Entry<Long, PartProfileEntry> entry =
                avail.getPrecEntry(time, true);
        if (entry != null) {
     ...
     */
    public ProfileEntry findPartStartTime(final int partId,
                                          final int reqPE,
        ...
        while (it.hasNext()) {
            ...
            while (ita.hasNext()) {
                ...
                intersect = intersect.intersection(
                        nextEntry.getAvailRanges(partId));
     ...
     * @see RangeList
     */
    public void allocatePartResourceRanges(final int partId,
     ...
     */
    public Collection<TimeSlot> getPartTimeSlots(final int partId,
                                                 final long startTime,
                                                 final long finishTime) {
        if (partId >= partitions.length || partId < 0) {
        ...
        ArrayList<Entry> subProfile =
                toArrayList(partId, startTime, finishTime);
     ...
     * be ignored. If you choose <code>1</code>, then all scheduling
     * options will be returned.
     * @param reqPEs the minimum number of PEs of the free time slots. Free
     * time slots whose numbers of PEs are smaller than <code>numPEs</code>
     * will be ignored. If you choose <code>1</code>, then all scheduling
     * options will be returned.
     * @return a collection with the time scheduling options in this
     * availability information object within the specified period of time.
     */
    public Collection<TimeSlot> getPartSchedulingOptions(final int partId,
                                                         final long startTime,
                                                         final long finishTime,
                                                         final int duration,
                                                         final int reqPEs) {
            ...
            while (slRgs != null && slRgs.getNumItems() > 0) {
                ...
                while (ita.hasNext() && !changed) {
                    ...
                    if ((slEnd - sStart) >= duration
                        ...
                        TimeSlot slot =
                                new TimeSlot(sStart, slEnd, slRgs.clone());
                        slots.add(slot);
                    }
                    changed = true;
                    slRgs = its;
     ...
     */
    public boolean addPartTimeSlot(final int partId,
                                   final long startTime,
                                   final long finishTime,
        ...
        // simulation clock time is required as the starting point
        // of the profile
        if (last.getTime() == startTime) {
     ...
     */
    private ArrayList<Entry> toArrayList(final int partId,
                                         final long startTime,
                                         final long finishTime) {
        checkElementIndex(partId, partitions.length,
                "Partition " + partId + " does not exist");
        ...
        if (it.hasNext()) {
            ...
            RangeList list = ent.getAvailRanges(partId) == null
                    ? null
                    : ent.getAvailRanges(partId).clone();
        ...
        while (it.hasNext()) {
            ...
            RangeList list = entry.getAvailRanges(partId) == null
                    ? null
                    : entry.getAvailRanges(partId).clone();
