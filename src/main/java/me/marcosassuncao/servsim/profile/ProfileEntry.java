 ...
 * profile. It contains the list of ranges of processors available
 * at a particular time. This time may represent either the start
 * time or completion of a job or advance reservation.
 *
...
public abstract class ProfileEntry implements Comparable<ProfileEntry> {
     ...
     */
    protected ProfileEntry(final long time) {
        this.time = time;
    }
     ...
     */
    public boolean setTime(final long time) {
        if (time < 0) {
            return false;
        }
     ...
     */
    public int compareTo(final ProfileEntry entry) {
