...
public enum SortAlgorithm {
    ...
    /** Comparator used to sort the jobs. */
    private final Comparator<Job> comparator;

    SortAlgorithm(final String description,
                  final Comparator<Job> comp) {
 ...
 * <li> If only one of the jobs has the status
 * {@link WorkUnit.Status#IN_EXECUTION}, then they are considered equal.<br>
 *      <b>NOTE:</b> This indicates that the comparator is being used
...
class OrderByPriority implements Comparator<Job> {
    public int compare(final Job o1, final Job o2) {
        ...
        if (comp != 0) {
            ...
            if (o1.getStatus() == IN_EXECUTION
                    && o2.getStatus() == IN_EXECUTION) {
                return Long.compare(o1.getSubmitTime(),
                        o2.getSubmitTime());
            } else if (o1.getStatus() == IN_EXECUTION
                    || o2.getStatus() == IN_EXECUTION) {
                return 0;
            } else {
                return Long.compare(o1.getSubmitTime(),
