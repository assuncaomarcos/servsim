...
public abstract class ReservationServerUser extends ServerUser {
    ...
    private static final Logger LOGGER =
            LogManager.getLogger(ReservationServerUser.class.getName());
     ...
     */
    public ReservationServerUser(final String name) {
        super(name);
    }
     ...
     */
    public Reservation reserveResources(final int srvId,
                                        final long delay,
                                        final long start,
                                        final long duration,
                                        final int numRes) {
    ...
    public Reservation reserveResources(final int srvId,
                                        ...
                                        final long duration,
                                        final int numRes) {
        return reserveResources(srvId, SimEvent.SEND_NOW,
                start, duration, numRes);
    }
    ...
    public void process(final SimEvent ev) {
        if (ev.type() == RESERVATION_RESPONSE) {
            try {
                ...
                LOGGER.error("Invalid reservation object.", e);
            }
    ...
    /**
     * Called when entity received a reservation(response)
     * back from a server.
