 ...
 *
 * @param <V> The type of entry the profile stores.
 */
public abstract class Profile<V extends ProfileEntry> {
     ...
     * @see ProfileEntry
     */
    protected Profile(final TreeMap<Long, V> avail) {
        this.avail.putAll(avail);
    }

    /**
     * Removes past entries from the availability profile, but keeps the
     * entry corresponding to the reference time provided, or
     * the entry preceding it if an entry with the provided
     * time does not exist.
     * @param refTime the reference time for removing the entries. <br>
     */
    public void removePastEntries(final long refTime) {
     ...
     * @return a {@link ProfileEntry} with the ranges available at
     * the given time.
     */
    public ProfileEntry checkAvailability(final long time) {
        Map.Entry<Long, V> entry = avail.getPrecEntry(time, true);
        return entry == null
                ? new Entry(time)
                : entry.getValue().clone(time);
    }
     ...
     * @param acceptLess defines whether less resources than
    ...
    public ProfileEntry checkAvailability(final int reqRes,
        ...
        while (it.hasNext()) {
            ...
            if (entry.getTime() >= finishTime
                    || (!acceptLess && intersec.getNumItems() < reqRes)) {
                break;
            }
    ...
    /**
     * Selects an entry able to provide enough processors to handle a job.
     * The method iterates the profile until it finds enough processors
     ...
     */
    public ProfileEntry findStartTime(final int reqPE,
    ...
    public void allocateResourceRanges(final RangeList selected,
                                       final long startTime,
     ...
     * @see TimeSlot
     */
    public Collection<TimeSlot> getTimeSlots(final long startTime,
                                             final long finishTime) {
     ...
     * slots whose time frames are smaller than <code>duration</code> will
     * be ignored. If you choose {@code 1}, then all
     * scheduling options will be returned.
     * @param reqPEs the minimum number of processors of the free time
     * slots. Free time slots whose numbers of processors are smaller
     * than {@code reqPEs} will be ignored. If you choose {@code 1},
     * then all scheduling options will be returned.
     *
     * @return a collection with the time scheduling options.
     * @see TimeSlot
     */
    public Collection<TimeSlot> getSchedulingOptions(final long startTime,
                                                     final long finishTime,
                                                     final int duration,
                                                     final int reqPEs) {
        ...
        while (it.hasNext()) {
            ...
            while (slRgs != null && slRgs.getNumItems() > 0) {
                ...
                while (ita.hasNext() && !changed) {
                    ...
                    if ((slEnd - sStart) >= duration
                            && slRgs.getNumItems() >= reqPEs) {
                        TimeSlot slot = new TimeSlot(sStart,
                                slEnd, slRgs.clone());
                        slots.add(slot);
                    }
                    changed = true;
                ...
                if (slRgs.getNumItems() == initialPE) {
                    if ((finishTime - sStart) >= duration
                            && slRgs.getNumItems() >= reqPEs) {
                        TimeSlot slot = new TimeSlot(sStart,
                                finishTime, slRgs.clone());
                        slots.add(slot);
                    }
    ...
    /**
     * Creates a string representation of the profile.
     ...
     * entry was replaced.
     */
    protected ProfileEntry add(final V entry) {
     ...
     * not found.
     */
    protected ProfileEntry getPrecedingValue(final long time) {
        Map.Entry<Long, V> entry = avail.getPrecEntry(time, true);
     ...
     * @return a collection with the time slots.
     */
    public Collection<TimeSlot> getTimeSlots(final long finishTime,
                            final ArrayList<Entry> subProfile) {
        ArrayList<TimeSlot> slots = new ArrayList<>();
    ...
    /**
     * Return a collection with the profile entries over a specified period.
     ...
     */
    public Collection<ProfileEntry> getAvailability(final long startTime,
                                                    final long finishTime) {
     ...
     */
    public ArrayList<Entry> toArrayList(final long startTime,
                                        final long finishTime) {
    ...
    protected static class Entry extends ProfileEntry {
        /** The ranges of resources available. */
         ...
         */
        protected Entry(final long time) {
            super(time);
         ...
         */
        protected Entry(final long time, final RangeList list) {
            super(time);
            ranges = list;
        }
         ...
         */
        @Override
        public ProfileEntry clone(final long time) {
        ...
        public String toString() {
            return MoreObjects.toStringHelper(ProfileEntry.class)
                       ...
                       .add("numProc", ranges != null
                               ? ranges.getNumItems()
                               : 0
                       )
                       .add("ranges", ranges != null
                               ? ranges
                               : "{[]}"
                       ).toString();
        }
         ...
         */
        public Entry create(final long time, final RangeList list) {
