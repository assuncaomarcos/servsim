...
/**
 * This class represents a simulation entity that handles events and
 * that can send events to other entities. When this class is extended,
 * there are a few methods that must be implemented:
 * <ul>
 *  <li> {@link #onStart()} is invoked by {@link Simulation} when
 *  the simulation starts. This is the place where the initial creation
 *  of events must be performed. For example, if the entity reads a
 *  workload from a file, reading of the workload file and the creation
 *  of simulation events will have to be triggered by this method.
 ...
 *  simulation finishes. If you want to save data in log files this is
 *  the method in which the corresponding code would be placed.
 * </ul>
 *
...
public abstract class SimEntity implements EventSink<SimEvent> {
    ...
    /**
     * The entity's name (for reporting and debugging).
     */
    ...
    /**
     * Determines whether the entity is active.
     */
     ...
     * is <code>null</code> or has size 0.
     */
    public SimEntity(final String name) throws IllegalArgumentException {
     ...
     * @return <code>true</code> if the entity is enabled;
    ...
    /**
     * This method is invoked by the {@link Simulation}
     * class whenever there is an event in the deferred queue,
     * which needs to be processed by the entity.
     ...
     * simulation finishes. If you want to save data in log
     * files this is the method in which the corresponding code would be placed.
     */
    public abstract void onShutdown();

    /**
     * Used by the entity to send a message (i.e. create an event)
     * to (for) another entity.
    ...
    protected void send(final int dest,
                        final long delay,
                        final Enum type,
